...
const DEFAULT_AGENT_WS_URL = "wss://xxz-xyzw.hortorgames.com/agent";
...
const normalizeInteger = (value: any, fallback: any) => {
...
const buildAgentLoginPayload = (tokenData: any, actualToken: string) => {
  const source =
  ...
  return compactObject({
    ...
    sessId: normalizeInteger(
      source.sessId,
      now * 100 + Math.floor(Math.random() * 100),
    ),
    connId: normalizeInteger(source.connId, now + Math.floor(Math.random() * 10)),
    isRestore: normalizeInteger(source.isRestore, 0),
    version:
      pickFirst(source, [
        "loginVersion",
        "clientVersion",
...
const normalizeAgentWsUrl = (url: string, loginPayload: Record<string, any>) => {
  try {
    const wsUrl = new URL(url);
    if (wsUrl.pathname !== "/agent") {
      return url;
    }

    const pValue = wsUrl.searchParams.get("p") || "";
    ...
    try {
      ...
      if (pValue) {
        existingPayload = { roleToken: pValue };
      }
    }

    const payload = buildAgentLoginPayload(
      { ...existingPayload, ...loginPayload },
      loginPayload.roleToken,
    );
    wsUrl.searchParams.set("p", JSON.stringify(payload));
...
export const useTokenStore = defineStore("tokens", () => {
  ...
  const attemptTokenRefresh = async (
    ...
    if (refreshSuccess) {
      ...
      const shouldReconnect =
        forceReconnect ||
        currentPath === "/tokens" ||
        currentPath === "/admin/account-management" ||
  ...
  const extractActualToken = (tokenData: any, fallback = ""): string => {
    if (!tokenData || typeof tokenData !== "object") {
      return `${fallback || tokenData || ""}`.trim();
    }

    for (const key of ["roleToken", "actualToken", "token", "gameToken", "p"]) {
      const value = tokenData[key];
      if (value === undefined || value === null) continue;

      if (typeof value === "object") {
        const nestedToken = extractActualToken(value);
        if (nestedToken) return nestedToken;
        continue;
      }

      const tokenValue = unwrapStoredTokenCandidate(value);
  ...
  const extractTokenFromUrlString = (value: string): string => {
    const tokenText = `${value || ""}`.trim();
    if (!tokenText) return "";

    const hasTokenParam = /(?:^|[?&])(p|token|roleToken|actualToken)=/.test(
      tokenText,
    );
    const looksLikeUrl = /^(https?|wss?):\/\//i.test(tokenText);
    if (!looksLikeUrl && !hasTokenParam) return "";

    const readParams = (query: string) => {
      try {
  ...
  const unwrapStoredTokenCandidate = (value: any): string => {
    if (value && typeof value === "object") {
      return extractActualToken(value);
    }

    let tokenValue = `${value || ""}`.trim();
    if (!tokenValue) return "";

    const tokenFromUrl = extractTokenFromUrlString(tokenValue);
    if (tokenFromUrl) {
      tokenValue = tokenFromUrl;
    }

    try {
      const parsedValue = JSON.parse(tokenValue);
      const nestedToken = extractActualToken(parsedValue, tokenValue);
      return nestedToken || tokenValue;
    } catch {
      return tokenValue;
    }
  };
  ...
  const parseBase64Token = (base64String: string) => {
    try {
      ...
      // 移除可能的前缀和空格
      const hasDataBase64Prefix = /^data:.*base64,/.test(base64String);
      ...
      const rawText = base64String.trim();
      let decoded = rawText;
      let tokenData;
      const tokenFromUrl = extractTokenFromUrlString(rawText);
      if (tokenFromUrl) {
        decoded = tokenFromUrl;
        try {
          tokenData = JSON.parse(tokenFromUrl);
        } catch {
          tokenData = { token: tokenFromUrl };
        }
      } else {
        try {
          ...
          try {
            ...
            const decodedTokenFromUrl = extractTokenFromUrlString(decoded);
            if (decodedTokenFromUrl) {
              decoded = decodedTokenFromUrl;
            }
            try {
            ...
            // 不是JSON格式，作为纯token处理，避免把真实roleToken误解成乱码
            tokenData = { token: hasDataBase64Prefix ? decoded : rawText };
          }
        }
      }

      // 提取实际token
      const actualToken = extractActualToken(tokenData, rawText);
  ...
  const importBase64Token = (
    ...
    const tokenData = {
      ...
      token: parseResult.data.actualToken, // 始终保存游戏连接需要的 roleToken
    };
  ...
  const createWebSocketConnection = async (
    ...
    try {
      ...
      let actualToken;
      let tokenDataForPayload: Record<string, any> = {};
      if (parseResult.success) {
        actualToken = parseResult.data.actualToken;
        tokenDataForPayload = parseResult.data;
      } else {
        if (validateToken(base64Token)) {
          actualToken = base64Token;
          tokenDataForPayload = { roleToken: base64Token };
        } else {
          throw new Error(`Token无效: ${parseResult.error}`);
        }
      }
      // 6. 构建WebSocket URL
      const loginPayload = buildAgentLoginPayload(
        tokenDataForPayload,
        actualToken,
      );
      const baseWsUrl = normalizeAgentWsUrl(DEFAULT_AGENT_WS_URL, loginPayload);

      const wsUrl = customWsUrl
        ? normalizeAgentWsUrl(customWsUrl, loginPayload)
        : baseWsUrl;
  ...
  const readTokenFromSourceUrl = async (sourceUrl: string) => {
    return scheduleAuthUserRequest(async () => {
      const response = await fetch(sourceUrl);
      if (!response.ok) {
        throw new Error(`source token request failed: ${response.status}`);
      }
  ...
  const refreshTokenForWebGame = async (gameToken: any) => {
    if (!gameToken) return "";

    if (gameToken.importMethod === "url" && gameToken.sourceUrl) {
      const token = await readTokenFromSourceUrl(gameToken.sourceUrl);
      if (token) {
        updateToken(gameToken.id, { token, lastUsed: new Date().toISOString() });
        return token;
      }
    }
    ...
    updateToken(gameToken.id, { lastUsed: new Date().toISOString() });
    return gameToken.token || "";
  };

  const prepareWebGameLogin = async (
    tokenId: string,
    options: { closeHelperConnection?: boolean } = {},
  ) => {
    const gameToken = gameTokens.value.find((t) => t.id === tokenId);
    if (!gameToken) {
    ...
    if (options.closeHelperConnection !== false && wsConnections.value[tokenId]) {
      await closeWebSocketConnectionAsync(tokenId);
    }

    const tokenText = await refreshTokenForWebGame(gameToken);
    const parseResult = parseBase64Token(tokenText);
    let actualToken = "";
    let tokenDataForPayload: Record<string, any> = {};
    ...
    if (parseResult.success) {
      actualToken = parseResult.data.actualToken;
      tokenDataForPayload = parseResult.data;
    } else if (validateToken(tokenText)) {
      actualToken = tokenText;
      tokenDataForPayload = { roleToken: tokenText };
    } else {
      throw new Error(parseResult.error || "Token invalid");
    }
    ...
    const manifestVersion =
      pickFirst(tokenDataForPayload, ["manifestVersion", "entryVersion"]) ||
      DEFAULT_WEB_GAME_MANIFEST_VERSION;
    const authData = compactObject({
      ...tokenDataForPayload,
      ...loginPayload,
      token: actualToken,
      actualToken,
      ...
      loginVersion,
      manifestVersion,
      name: gameToken.name || tokenDataForPayload.name || "",
      server: gameToken.server || tokenDataForPayload.server || "",
    });
    ...
    return {
      ...
      openId: pickFirst(authData, ["openId", "openid"]),
      name: gameToken.name || tokenDataForPayload.name || "",
      server: gameToken.server || tokenDataForPayload.server || "",
    };
  };
