<template>
  <div ref="containerRef" class="orb-background" aria-hidden="true" />
</template>
...
onMounted(() => {
  ...
  try {
    ...
    console.warn("Orb background could not start.", error);
    return;
  }
  ...
  const program = new Program(gl, {
    ...
    uniforms: {
      ...
      backgroundColor: { value: colorToVec3(props.backgroundColor) },
    },
  });
  const mesh = new Mesh(gl, { geometry, program });

  resizeObserver = new ResizeObserver(() => resize(program));
  resizeObserver.observe(container);
  resizeHandler = () => resize(program);
  ...
  const updateBackgroundUniform = () => {
    if (props.backgroundColor === lastBackgroundColor) return;

    const bg = colorToVec3(props.backgroundColor);
    program.uniforms.backgroundColor.value.set(bg.x, bg.y, bg.z);
    lastBackgroundColor = props.backgroundColor;
  };
